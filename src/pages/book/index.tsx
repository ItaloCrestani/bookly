...
export function BookDetails() {
  ...
  return (
    <div className="flex text-(--text-1) ">
      <div className="w-60 shrink-0 mr-10 text-(--text-2) font-medium">
        <div className="w-full h-88 mb-6 rounded-xl overflow-hidden">
        ...
        <div className="flex flex-col gap-2">
          <p className="text-[14px] font-semibold">Adicionar à minha estante</p>

          <button
          className={`flex items-center gap-2 px-4 py-3 bg-(--bg-read) border border-(--border-read) rounded-lg hover:bg-(--hover-read) transition-colors ${loading ? 'cursor-not-allowed' : 'cursor-pointer'}`} 
          disabled={loading}
          onClick={AddRead}
          >
            <CiBookmark size={30} color="var(--text-read)" />
            <div className="flex flex-col items-start gap-0.5">
              <p className="text-[14px] text-(--text-read)">Quero ler</p>
              <p className="text-[10px]">Adicionar à sua lista de desejos</p>
            </div>
          </button>

          <button 
          className={`flex items-center gap-2 px-4 py-3 bg-(--bg-reading) border border-(--border-reading) rounded-lg hover:bg-(--hover-reading) transition-colors ${loading ? 'cursor-not-allowed' : 'cursor-pointer'}`} 
          disabled={loading}
          onClick={AddReading}
          >
            <HiOutlineBookOpen size={30} color="var(--text-reading)" />
            <div className="flex flex-col items-start gap-0.5">
              <p className="text-[14px] text-(--text-reading)">Estou lendo</p>
              <p className="text-[10px]">Acompanhar sua leitura</p>
            </div>
          </button>

          <button 
          className={`flex items-center gap-2 px-4 py-3 bg-(--bg-readed) border border-(--border-readed) rounded-lg hover:bg-(--hover-readed) transition-colors ${loading ? 'cursor-not-allowed' : 'cursor-pointer'}`} 
          disabled={loading}
          onClick={AddReaded}
          >
            <IoIosCheckmarkCircleOutline size={30} color="var(--text-readed)" />
            <div className="flex flex-col items-start gap-0.5">
              <p className="text-[14px] text-(--text-readed)">Já li</p>
              <p className="text-[10px]">Marcar como concluído</p>
            </div>
          </button>
        </div>
      </div>

      <div className="w-160 min-w-0 overflow-hidden">
        {book?.genre && (
